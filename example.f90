program main
   use  m_unify_zmat
   implicit none
   integer, parameter :: N=750, lwork = 3*N
   ...
   theta = 0.3
   H = matmul(H, givens(1,2, theta, N))
   H = matmul(givens(1,2, -theta, N), h) 
   theta = 0.4
   H = matmul(H, givens(3,7, theta, N))
   H = matmul(givens(3,7, -theta, N), h) 
   theta = 0.5
   H = matmul(H, givens(2,6, theta, N))
   H = matmul(givens(2,6, -theta, N), h) 
...
contains
   function givens(i,j,theta, N) result(mat)
      implicit none 
